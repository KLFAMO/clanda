...
def calc(*, from_mjd, to_mjd, create_cmp_file: bool = False, plot: bool = False, **kwargs):
    ...
    gts = get_gts_single_mjd(
        datasets=["comb_hydro_698", "sr1_aom_cor", "comb_frep_dm"],
    ...
    gts.math_mts_and_number('add', 'fc_aom', 99.078, 'mfc')  # atoms correction + total shift  temporary, without total shift
    ...
    gts.append_mtserie(
        mts_name='avg',
        mts=gts.mts_dict['diffr_rel'].resample(period_s=2600)
    )
